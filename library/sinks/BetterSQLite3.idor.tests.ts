...
export function createBetterSQLite3IdorTests(versionPkgName: string) {
  t.test("IDOR protection for BetterSQLite3", async (t) => {
    const agent = startTestAgent({
      wrappers: [new BetterSQLite3()],
      rewrite: {
        "better-sqlite3": versionPkgName,
      },
    });

    let betterSqlite3 = require(versionPkgName);
    if (isEsmUnitTest()) {
