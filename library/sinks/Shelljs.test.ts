...
t.test("it detects shell injections", async () => {
  ...
  if (error instanceof Error) {
    t.same(
      error.message,
      "Zen by Aikido has blocked a shell injection: shelljs.exec(...) originating from body.myTitle"
    );
  }
});
...
t.test("it detects async shell injections", async () => {
  ...
  if (error instanceof Error) {
    t.same(
      error.message,
      "Zen by Aikido has blocked a shell injection: child_process.exec(...) originating from body.myTitle"
    );
  }
...
t.test("it prevents path injections using cat", async () => {
  ...
  if (error instanceof Error) {
    t.same(
      error.message,
      "Zen by Aikido has blocked a path traversal attack: fs.existsSync(...) originating from body.myTitle"
    );
  }
});
