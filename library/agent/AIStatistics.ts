...
export class AIStatistics {
  ...
  getStats() {
    return Array.from(this.calls.values()).map((stats) => {
      const routes: AIRouteStats[] = [];
      for (const routeKey of stats.routesLRU.keys()) {
        const route = stats.routesLRU.get(routeKey);
        if (route) {
          routes.push({
            path: route.path,
            method: route.method,
