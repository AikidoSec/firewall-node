...
t.test("it connects and receives config-updated events", async (t) => {
  ...
  try {
    ...
    t.same(JSON.parse(events[0].data), { serviceId: 1, configUpdatedAt: 100 });
    ...
    t.same(JSON.parse(events[1].data), { serviceId: 1, configUpdatedAt: 200 });
  } finally {
