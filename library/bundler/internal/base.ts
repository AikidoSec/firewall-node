...
export const fileMatchRegex = /\.(js|ts|cjs|mjs|jsx|tsx)$/;

export function onBuildStart() {
  if (!initialized) {
    initialized = true;
...
export async function onBuildEnd(
  processedBundlerOpts: ProcessedBundlerOptions | undefined,
  ...
  if (processedBundlerOpts.outputFormat === "cjs" && importFound === false) {
    throw new Error(
