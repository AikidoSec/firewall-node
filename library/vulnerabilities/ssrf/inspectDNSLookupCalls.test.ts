...
t.test("it blocks lookup in blocking mode", (t) => {
  ...
  runWithContext(context, () => {
    wrappedLookup("localhost", (err, address) => {
      ...
      t.same(
        err.message,
        "Zen by Aikido has blocked a server-side request forgery: operation(...) originating from body.image"
      );
  ...
  await Promise.all([
    new Promise<void>((resolve) => {
      wrappedLookup("imds.test.com", { family: 4 }, (err, addresses) => {
        ...
        t.same(
          err.message,
          "Zen by Aikido has blocked a server-side request forgery: operation(...) originating from unknown source"
        );
        t.same(addresses, undefined);
        resolve();
      });
