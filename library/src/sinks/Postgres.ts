...
export function inputPossibleSql(input: string): boolean {
  const regex = /(?<![a-z0-9])(SELECT|CREATE|DROP|DATABASE|UPDATE|DELETE|ALTER|GRANT|SAVEPOINT|COMMIT|ROLLBACK|TRUNCATE|OR|AND|UNION|AS|WHERE)(?![a-z0-9])|('|"|`)|(\}|\{|\}|\*)/gim; // Needs to be an actual regex
