...
export function inputPossibleSql(input: string): boolean {
  const regex = /(SELECT|CREATE|DROP|DATABASE|UPDATE|DELETE|ALTER|GRANT|SAVEPOINT|COMMIT|ROLLBACK|TRUNCATE)|('|"|`)|(\}|\{|\}|\*)|(OR|AND|UNION|AS|WHERE)/gim; // Needs to be an actual regex
