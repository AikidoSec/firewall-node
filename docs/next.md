...
```Dockerfile
CMD node -r @aikidosec/firewall/instrument server.js
```
...
```json
{
  "scripts": {
    ...
    "dev": "NODE_OPTIONS='-r @aikidosec/firewall/instrument' next dev",
    "start": "NODE_OPTIONS='-r @aikidosec/firewall/instrument' next start"
  }
}
```
...
```sh
AIKIDO_BLOCK=true node -r @aikidosec/firewall/instrument .next/standalone/server.js
```
...
```sh
AIKIDO_DEBUG=true node -r @aikidosec/firewall/instrument .next/standalone/server.js
```
