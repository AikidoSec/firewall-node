...
test("it blocks request in blocking mode (ESM)", async () => {
  const server = spawn(
    `node`,
    ["-r", "@aikidosec/firewall/instrument", "./app-esm.mjs", port3],
    {
      cwd: esmAppDir,
      env: {
        ...process.env,
...
test("it does not block request in monitoring mode (ESM)", async () => {
  const server = spawn(
    `node`,
    ["-r", "@aikidosec/firewall/instrument", "./app-esm.mjs", port4],
    {
      cwd: esmAppDir,
      env: {
        ...process.env,
...
test("it does not block request if aikido disable is set (ESM)", async () => {
  const server = spawn(
    `node`,
    ["-r", "@aikidosec/firewall/instrument", "./app-esm.mjs", port5],
    {
      cwd: esmAppDir,
      env: {
        ...process.env,
